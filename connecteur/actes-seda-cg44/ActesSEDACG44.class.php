...
class ActesSEDACG44 extends SEDAConnecteur {
	...
	public function  setConnecteurConfig(DonneesFormulaire $seda_config){
		$this->authorityInfo = array(
				...
				"siret_entite" =>  $seda_config->get('siret_entite'),
		);
	}
	...
	public function getBordereau(array $transactionsInfo){
		//echo "<pre>";print_r($transactionsInfo);echo "<pre>";exit;
		...
		$archiveTransfer->Comment = "Transfert des actes administratifs envoyés au contrôle de légalité via le tiers de télétransmission Slow  - ".date('Y');
		$archiveTransfer->Date = date('c');//"2011-08-12T11:03:32+02:00";
		$archiveTransfer->TransferIdentifier = "44-".$transactionsInfo['numero_acte_collectivite']."-".time();
		$archiveTransfer->TransferIdentifier['schemeAgencyName'] = "S2LOW";
		
		$archiveTransfer->TransferringAgency->Description = "Plateforme S2LOW de l'ADULLACT";
		$archiveTransfer->TransferringAgency->Identification = $this->authorityInfo['sae_id_versant'];
		$archiveTransfer->TransferringAgency->Name = "S2LOW";

		$archiveTransfer->ArchivalAgency->Description = "Archives départementales de Loire Atlantique";
		$archiveTransfer->ArchivalAgency->Identification = $this->authorityInfo['sae_id_archive'];		
		$archiveTransfer->ArchivalAgency->Name = "Archives départementales de Loire Atlantique";
		...
		$archiveTransfer->Contains->ArchivalAgreement = "ACTES-S2LOW-2014";
		...
		$archiveTransfer->Contains->ArchivalProfile['schemeAgencyName'] = "Profil élaboré par les Archives départementales de la Loire Atlantique";
		...
		$archiveTransfer->Contains->Name = "Contrôle de légalité : nature de l'acte en date du date de décision de l'acte du Conseil général de la Loire Atlantique.";
		...
		$archiveTransfer->Contains->ContentDescription->OriginatingAgency->Description = "Service producteur des archives";
		$archiveTransfer->Contains->ContentDescription->OriginatingAgency->Identification = $this->authorityInfo['sae_originating_agency'];
		$archiveTransfer->Contains->ContentDescription->OriginatingAgency->Identification['schemeName'] = "SIRET";
		$archiveTransfer->Contains->ContentDescription->OriginatingAgency->Identification['schemeAgencyName'] = "INSEE";
		$archiveTransfer->Contains->ContentDescription->OriginatingAgency->Name = "Conseil général de la Loire Atlantique";
	
	
		$archiveTransfer->Contains->ContentDescription->ContentDescriptive[0]->KeywordContent = "Conseil général de la Loire Atlantique";
		$archiveTransfer->Contains->ContentDescription->ContentDescriptive[0]->KeywordReference = $this->authorityInfo['siret_entite'];
		$archiveTransfer->Contains->ContentDescription->ContentDescriptive[0]->KeywordReference['schemeName'] = "SIRET";
		...
		$archiveTransfer->Contains->Contains[0] = $this->getContainsElement("Acte soumis au contrôle de légalité",$transactionsInfo);
	...
	private function getContainsElement($description,$transactionInfo){
		$contains = new ZenXML("Contains");		
		$contains->DescriptionLevel = "file";
		$contains->DescriptionLevel['listVersionID'] = "edition 2009";
		$contains->Name = $description;
		$contains->TransferringAgencyObjectIdentifier = $transactionInfo['numero_acte_collectivite'];
		$contains->TransferringAgencyObjectIdentifier['schemeAgencyName'] = "Conseil général de la Loire Atlantique";
		return $contains;
	}
